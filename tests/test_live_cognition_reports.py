"""Live cognition reports: all generated, safety boundaries, ClaimGuard-safe."""
...
def test_all_reports_generated(tmp_path):
    state = _run(tmp_path)
    base = os.path.join(state, "cognition", "reports")
    for name in ("LIVE_COGNITION_REPORT.md", "SIGN_INPUT_REPORT.md",
                 "COGNITION_TRACE_REPORT.md", "ANTICIPATION_REPORT.md",
                 "UNCERTAINTY_REPORT.md", "RELATION_TRAVERSAL_REPORT.md",
                 "INTERNAL_SIMULATION_REPORT.md",
                 "PREDICTION_ASSESSMENT_REPORT.md",
                 "COGNITION_CONTAMINATION_REPORT.md",
                 "COGNITION_READINESS_GATE_REPORT.md",
                 "LIVE_COGNITION_MEMORY_REPORT.md"):
        assert os.path.isfile(os.path.join(base, name)), name


def test_safety_boundaries_in_report(tmp_path):
    state = _run(tmp_path)
    md = open(os.path.join(state, "cognition", "reports",
                           "LIVE_COGNITION_REPORT.md")).read().lower()
    assert "private signs are not proof of language or understanding" in md
    assert "cognition traces are not proof of consciousness or agency" in md
...
def test_reports_are_claimguard_safe(tmp_path):
    if not _HAS_GUARD:
        return
    state = _run(tmp_path)
    base = os.path.join(state, "cognition")
