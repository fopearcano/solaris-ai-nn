"""Live cognition record: generated, disclaimer present, blockers visible."""
...
def test_record_generated(tmp_path):
    ...
    md = [f for f in os.listdir(reports)
          if f.startswith("LIVE_COGNITION_RECORD_") and f.endswith(".md")]
    assert md
...
def test_disclaimer_present(tmp_path):
    ...
    reports = os.path.join(state, "cognition", "reports")
    md = [f for f in os.listdir(reports)
          if f.startswith("LIVE_COGNITION_RECORD_") and f.endswith(".md")][0]
...
def test_blockers_visible(tmp_path):
    ...
    json.dump(feeder_registry_template(), open(os.path.join(
        state, "feeders", "FEEDER_REGISTRY.json"), "w"))
    rt = FirstLiveCognitionRuntime(state_dir=state,
                                   require_birth_certificate=True,
                                   require_observation_stability=True,
                                   require_live_signs=True)
    result = rt.run()
