"""Developmental-life safety -- long-horizon development stays operational/honest.

:class:`DevelopmentalLifeSafetyValidator` enforces the hard rules this layer can
never break: no biological-life claims, no consciousness/sentience/personhood
claims, no agency/free-will claims, no emotion/feeling claims, no human teaching
loop, no real-world actuation, no hardware/feeder control, no network/shell/browser/
OS, no source modification, no sensory text as a command, no human label as ground
truth, no unbounded runtime, and no deletion of regressions/plateaus/failures.

"Life cycle", "maturation", and "life history" here are operational runtime
language -- NOT biological life, consciousness, or biography.
"""
...
HARD_RULES = (
    ...
    "no deletion of regressions/plateaus/failures",
)
...
@dataclass
class DevelopmentalLifeSafetyValidator:
    """Validates that long-horizon development stays operational and bounded."""
    ...
    def _finish(self, check: str,
        ...
        return DevelopmentalSafetyReport(safe=not violations, check=check,
                                         violations=violations)

    def validate_operation(self, operation: str) -> DevelopmentalSafetyReport:
    ...
    def validate_bounded(self, max_ticks: Any,
                         max_runtime_s: Any) -> DevelopmentalSafetyReport:
        unbounded = (not max_ticks) and (not max_runtime_s)
        return self._finish("bounded",
                            ["no unbounded runtime"] if unbounded else [])
    ...
    def validate_claim_text(self, text: str) -> DevelopmentalSafetyReport:
