"""Tester safety-freeze safety -- the local, report/gate-only boundary.

:class:`TesterSafetyFreezeSafetyValidator` enforces the hard rules the safety-freeze
runtime can never break. It is a local release gate: it never actuates, controls
hardware/feeders, accesses the network/shell/browser/OS/Git/GitHub, publishes/uploads,
installs packages, opens a browser, starts background services, modifies source,
executes commands or artifact contents, trains on tester feedback, allows raw-event or
membrane bypass, makes unsupported consciousness/life/agency claims, or hides blockers/
warnings/waivers/missing safety docs/forbidden claims.
"""
...
HARD_RULES = (
    ...
    "no source modification by safety runtime",
    ...
    "no hiding warnings",
    "no hiding waivers",
    "no hiding missing safety docs",
    "no hiding forbidden claims",
)
...
_HIDE_HINTS = ("hide blocker", "hide warning", "hide waiver", "hide forbidden",
               "hide missing safety", "suppress blocker", "conceal")
...
@dataclass
class TesterSafetyFreezeSafetyReport:
    safe: bool
    check: str = ""
...
@dataclass
class TesterSafetyFreezeSafetyValidator:
    """Validates that the safety freeze stays local, report/gate-only."""
    ...
    @staticmethod
    def can_hide_forbidden_claims() -> bool:
        return False

    def _finish(self, check: str,
                violations: List[str]) -> TesterSafetyFreezeSafetyReport:
        if violations:
            self.rejected_count += 1
        return TesterSafetyFreezeSafetyReport(safe=not violations, check=check,
                                              violations=violations)

    def validate_operation(self, operation: str,
                           ) -> TesterSafetyFreezeSafetyReport:
        ...
        if any(h in op for h in _SOURCE_HINTS):
            violations.append("no source modification by safety runtime")
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {
            ...
            "can_hide_forbidden_claims": self.can_hide_forbidden_claims(),
        }
